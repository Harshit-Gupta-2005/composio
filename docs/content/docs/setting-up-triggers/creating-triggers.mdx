...
The walkthrough below uses Slack as the example. For setup notes specific to each toolkit, see its FAQ section — e.g., [Slack](/toolkits/slack), [Notion](/toolkits/notion).
...
Call the [Webhook Endpoints API](/reference/api-reference/webhooks) with the toolkit slug and your OAuth `client_id`:

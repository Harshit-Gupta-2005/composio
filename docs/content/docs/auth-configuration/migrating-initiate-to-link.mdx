...
## `allow_multiple` carries over unchanged

`link()` accepts `allow_multiple` (Python) / `allowMultiple` (TypeScript) with the same default (`False`), same semantics, and same exception (`ComposioMultipleConnectedAccountsError`) as `initiate()`. Pass the flag through unchanged — see the [SDK link allow_multiple changelog](/docs/changelog/2026/04/28) for the full release note. Requires `@composio/core` v0.8.1+ (TypeScript) or `composio` v0.12.1+ (Python).

...
<Callout type="warn">
Shared Connections APIs are experimental and may change in future releases. All new fields are nested under an `experimental` block on the wire, and the SDK methods that operate on them (`experimental_updateAcl`, `composio.experimental.update_acl`) live under an experimental namespace. The code samples below show the experimental shape — pin a specific SDK version if you depend on the current contract.
</Callout>
...
Pass an `experimental` block to `link()` containing `account_type: "SHARED"` (and optionally an initial ACL). Omit the ACL block to keep the default deny-by-default state (only the creator can use it until you grant access).
...
The table below shows the inner shape of `acl_config_for_shared` — wrap it inside the `experimental` block at the call site.

| Goal | `acl_config_for_shared` |
|---|---|
...
// ---cut---
// Open access to everyone.
await experimental_updateAcl(composio, "ca_gmail_shared", {
  allowAllUsers: true,
});
...
// ---cut---
try {
  await experimental_updateAcl(composio, "ca_private_gmail", {
    allowAllUsers: true,
  });
} catch (error) {
